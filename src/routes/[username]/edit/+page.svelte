<script lang="ts">
    ...
    $: href_bio = `/${$userData?.username}/bio`;
  ...
  <main class="max-w-xl mx-auto">
    {#if $userData?.username == $page.params.username}
      ...
      <button
        on:click={() => gotoProf()}
        class="btn btn-link block mx-auto my-4 mb-0 ps-0"
        >
          See Profile
        </button>
      ...
      {:else}
        ...
        <button
        on:click={() => gotoBio()}
        class="btn btn-outline btn-primary block mx-auto my-4"
        >
          Edit Bio
        </button>
      {/if}
